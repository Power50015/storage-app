<template>
  <AppLayout title="مرتجع فاتوره صادره">
    <SectionTemplate class="pb-0">
      <h2 class="font-bold text-xl mb-5">مرتجع الفاتورة الصادره</h2>
      <div class="flex justify-between">
        <h2 class="px-2 print:w-full print:mb-3">
          المورد : {{ outgoingInvoice.people.name }}
        </h2>
    ...
    <FormSection
      ...
      :formData="form"
      formRoute="returned-outgoing-invoice.update"
      toastMsg="تم تسجيل مرتجع الفاتوره"
      :toastDescription="`تم تسجيل مرتجع الفاتوره ${outgoingInvoice.id}`"
    >
      <!-- Date -->
      <InputDate
        v-model="form.date"
      ...
      <div class="w-full my-5 dark:bg-[#151521] p-2 bg-[#F5F8FA]">
        ...
        <div class="flex justify-between">
          <h2 class="px-3 dark:text-gray-300 title font-bold mb-4">
            الإجمالى  :<br />{{  (totalPrice - outgoingInvoice.discount).toFixed(2) }}
          </h2>
          <h2 class="px-3 dark:text-gray-300 title font-bold mb-4">
            الإجمالى قبل الخصم :<br />{{ totalPrice.toFixed(2) }}
          </h2>
          <h2 class="px-3 dark:text-gray-300 title font-bold mb-4">
            الخصم :<br />{{ outgoingInvoice.discount }}
          </h2>
        </div>
      </div>
    </FormSection>    
...
provide(
  ...
  readonly([
    ...
    {
      index: 2,
      linkTitle: "مرتجع فاتوره صادره رقم : " + outgoingInvoice.id,
      linkRoute: "#",
    },
  ])
);
...
function total() {
  ...
  totalPrice.value =
    outgoingInvoice.total_before_discount - totalPrice.value;
}
