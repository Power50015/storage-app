<template>
  ...
  <div class="grid gap-x-4 gap-y-2 grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
    <div
      ...
      "
    >
      <destructible-goods-item
        :item="item"
        @FormSuccess="getGoodsData(true)"
      />
    </div>
  </div>
...
import DestructibleGoodsItem from "./DestructibleGoodsItem.vue";
...
function getGoodsData(reload = false, page = 1) {
  ...
  axios
    .get(route("destructible-goods-action.index"), {
      params: {
