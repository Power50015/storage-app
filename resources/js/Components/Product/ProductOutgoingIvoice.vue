    ...
    <tbody>
      <tr v-for="i in outgoing_invoices" :key="i.number">
        <td>
          ...
          >
            <Link
              :href="route('outgoing-invoice.show', i.outgoing_invoice_id)"
              class="data hover:text-[#009ef7]"
            >
              <h3 class="">رقم الفاتوره : {{ i.outgoing_invoice_id }}</h3>
              <div class="text-sm">{{ i.outgoing_invoice.people.name }}</div>
              <div class="text-sm pb-3">
                نوع الدفع :
                <span v-if="i.outgoing_invoice.pay_type == 0">على الحساب</span>
                <span v-if="i.outgoing_invoice.pay_type == 1">كاش</span>
              </div>
              <div class="text-md text-[#009ef7]">
                {{ dateFormat(i.outgoing_invoice.date) }}
              </div>
...
const outgoingIvoice = props.outgoing_invoices;
