<template>
  ...
  <div v-for="item in actions" :key="item.index">
    <div v-if="item.dataType == 'IncomingInvoice'">
      ...
      >
        <div class="w-full">
          ...
          <h2>المخزن : {{ item.warehouse.name }}</h2>
          <div class="flex mt-3">
            <img
    ...
    <div v-if="item.dataType == 'ReturnedIncomingInvoice'">
      ...
      >
        <div class="w-full">
          <h2>رقم البيان : {{ item.returned_incoming_invoice.id }}</h2>
          <h2>المخزن : {{ item.warehouse.name }}</h2>
          <div class="flex mt-3">
            <img
              :src="`/storage/${item.product.image}`"
        ...
        <div class="w-full flex justify-center">
          <BadgeSuccess>مرتجع وارده</BadgeSuccess>
        </div>
        <div class="w-full">
          <div class="flex">
            الكميه : {{ item.quantity }} * السعر : {{ item.price }}
    ...
    <div v-if="item.dataType == 'ReturnedIncomingInvoiceKit'">
      ...
      >
        <div class="w-full">
          <h2>رقم البيان : {{ item.returned_incoming_invoice.number }}</h2>
          <h2>المخزن : {{ item.warehouse.name }}</h2>
          <div class="flex mt-3">
            <img
              :src="`/storage/${item.kit.image}`"
        ...
        <div class="w-full">
          ...
          <div>إجمالى القطع الغيار : {{ item.quantity * item.price }}</div>
          <div>
            إجمالى الفاتوره :
            {{ item.returned_incoming_invoice.total }}
          </div>
    ...
    <div v-if="item.dataType == 'OutgoingInvoiceKit'">
      ...
      >
        <div class="w-full">
          <h2>رقم الفاتوره : {{ item.id }}</h2>
          <h2>المخزن : {{ item.outgoing_invoice.name }}</h2>
          <div class="flex mt-3">
            <img
              :src="`/storage/${item.kit.image}`"
              :alt="item.kit.name"
    ...
    <div v-if="item.dataType == 'ReturnedOutgoingInvoiceKit'">
      ...
      >
        <div class="w-full">
          <h2>رقم الفاتوره : {{ item.id }}</h2>
          <h2>المخزن : {{ item.returned_outgoing_invoice.name }}</h2>
          <div class="flex mt-3">
            <img
              :src="`/storage/${item.kit.image}`"
              :alt="item.kit.name"
